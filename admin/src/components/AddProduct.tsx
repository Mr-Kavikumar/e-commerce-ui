...
const formSchema = z.object({
  name: z
    ...
    .max(50),
  shortDescription: z.string().min(1 , {message:"Short Descritpion is Required!"}).max(60),
  description: z.string().min(1 , {message:"Descritpion is Required!"}),
                      ...
                      <div className="grid grid-cols-3 gap-4 my-2">
                        {sizes.map((size)=>(
                          <div className="flex items-center gap-2" key={size}>
                            <Checkbox id={size} checked={field.value?.includes(size)}
                      ...
                      <div className="grid grid-cols-3 gap-4 my-2">
                        {colors.map((color)=>(
                          <div className="flex items-center gap-2" key={color}>
                            <Checkbox id={color} checked={field.value?.includes(color)}
                            onCheckedChange={(checked)=>{
                              const currentValues = field.value || []
                              if(checked){
